...
public class SolWeb3Connector : MonoBehaviour
{
    public static SolWeb3Connector instance;
    ...
    public static string mainNetAdress = "https://fragrant-billowing-haze.solana-mainnet.quiknode.pro/";

    public static string webSocketDevNetAdress = "wss://api.devnet.solana.com";
    public static string webSocketTestNetAdress = "wss://api.testnet.solana.com";
    ...
    //public GameObject NftImageSelector;
    ...
    /// <param name="account">Forwarded account for which we want to return data</param>
    /// <returns></returns>
    public async Task<AccountInfo> GetAccountData(string publicKey)
    {
        RequestResult<ResponseValue<AccountInfo>> result = await activeRpcClient.GetAccountInfoAsync(publicKey);
        if (result.Result != null && result.Result.Value != null)
        {
            return result.Result.Value;
        }
    ...
    /// <returns></returns>
    public async Task<TokenAccount[]> GetOwnedTokenAccounts(string walletPubKey, string tokenMintPubKey = "", string tokenProgramPublicKey = "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA")
    {
    ...
    private static extern void drgweb3_init();
#endif
    public static SolWeb3Connector sInstance;
    public string WalletAddress { get; private set; }


    public void WalletConnected(string f_data)
    {
        //GameObject.Find("dragonWeb3Locker").SetActive(false);

        string[] data = f_data.Split('|');
        WalletAddress = data[0];
        LobbyController.Instance.walletAddress = WalletAddress;
    ...
    void Start()
    {
        ...
        drgweb3_init();
#else
        WalletConnected("J2WJMWTYevCD6PChMG49cnc2GrxBXndUSa1Sb5JyYj59|SignMessage");
#endif
    }
