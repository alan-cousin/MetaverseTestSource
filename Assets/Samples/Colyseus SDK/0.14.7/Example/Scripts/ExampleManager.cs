...
public class ExampleManager : ColyseusManager<ExampleManager>
{
    ...
    public Dictionary<string, string> userAvataUrls = new Dictionary<string, string>();
    /// <summary>
    ...
    {
        if (client == null) CreateClient(_colyseusSettings.WebSocketEndpoint);
        client.Login(walletAddress, signedMsg);
    }
    ...
    public async void JoinOrCreateNewRoom(string roomID)
    {
        _roomController.JoinOrCreateRoom();
    }
...
#region Remote Function Call
    ...
    public static void SendChatMsg(Dictionary<string, string> msg)
    {
        NetSend("onChatMsg", msg);
    }

    public static void SendFurnitureData(PieceData fData)
    {
        NetSend("onFurniture", fData);
    }

    public static void RequestAgoraTokenBuild()
    {
        NetSend("onTokenBuild");
    }
    /// <summary>
