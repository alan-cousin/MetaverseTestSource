...
public class ExampleNetworkedEntityView : ColyseusNetworkedEntityView
    {
        ...
        public string usingPieceKey = "";
        public string preUsingPieceKey = "";
        ...
        protected virtual void UpdateStateFromView()
        {
            ...
            state.usingPieceKey = usingPieceKey;
        ...
        protected virtual void Update()
        {
            ...
            else if (!IsMine)
            {
                ...
                string[] pkt = state.usingPieceKey.Split('-');
                foreach (GameObject p in pieces)
                {
                    if (pkt.Length == 0 || preUsingPieceKey == state.usingPieceKey) break;
                    if (p.GetComponent<FurniturePiece>().uniquePieceKey != pkt[0]) continue;
                    if (pkt[1] == "stand")
                    {
                        if (p.transform.GetChild(1).childCount != 0)
                            Destroy(p.transform.GetChild(1).GetChild(0).gameObject);
                    ...
                    {
                        if (transform.GetChild(0).GetChild(0).gameObject.GetComponent<Animator>() == null)  //check updating child
                             return;
                        GameObject originObj = Instantiate(p.GetComponent<PieceTrigger>().mainBodyPrefab) as GameObject;
                        GameObject mainBody = Instantiate(transform.GetChild(0).GetChild(0).gameObject, p.transform.GetChild(1).position, p.transform.GetChild(1).rotation) as GameObject;
                        mainBody.GetComponent<Animator>().runtimeAnimatorController = originObj.GetComponent<Animator>().runtimeAnimatorController;
                        mainBody.transform.SetParent(p.transform.GetChild(1));
                        mainBody.GetComponent<Animator>().enabled = true;
                        Destroy(originObj);
                        mainBody.AddComponent<AvataControl>();
                        mainBody.GetComponent<AvataControl>().SetAnimType((int)(p.GetComponent<PieceTrigger>().pieceType));
                        gameObject.transform.GetChild(0).gameObject.SetActive(false);
                        gameObject.GetComponent<Rigidbody>().useGravity = false;
                        gameObject.GetComponent<CapsuleCollider>().enabled = false;
                    }
                    else if (pkt[1] == "watchon")
                    {
                        GameObject videoPane = Instantiate(p.GetComponent<PieceTrigger>().videoPlane, p.transform.position, p.transform.rotation) as GameObject;
                        videoPane.transform.SetParent(p.transform);
                    }
                    else if (pkt[1] == "watchoff")
                    {
                        Destroy(p.transform.GetChild(1).gameObject);
                    }
                }
                if (pkt.Length == 2 && pkt[0].Contains("door") && preUsingPieceKey != state.usingPieceKey)
                {
                      GameObject cashObj = GameObject.FindGameObjectWithTag(pkt[0]);   
                      if (pkt[1] == "doorin") cashObj.GetComponent<Animator>().SetTrigger(pkt[0] + "_in");
                      if (pkt[1] == "doorout") cashObj.GetComponent<Animator>().SetTrigger(pkt[0] + "_out");
                }
                preUsingPieceKey = state.usingPieceKey;
                //GetComponent<Animator>().SetFloat("InputMagnitude", Mathf.Clamp(velLen, 0f, 0.5f));
            }

            //float jumpHei = localPositionDelta.y;
        }
