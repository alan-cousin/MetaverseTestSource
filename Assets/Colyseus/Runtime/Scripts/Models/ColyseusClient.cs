...
namespace Colyseus
{
    ...
    public class ColyseusClient
    {
        ...
        public async void Login(string account, string signedMsg)
        {
            Dictionary<string, object> data = new Dictionary<string, object>();

            data.Add("account", account);
            data.Add("signedMsg", signedMsg);

            string json = 
              await ColyseusRequest.Request("POST", $"api/auth/login", data, null); //req.downloadHandler.text;
             
            //List<string> nftRoomIdList = JsonUtility.FromJson<List<string>>(json);
        }
