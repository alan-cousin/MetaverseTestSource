...
public class ManagerEditor : Editor{
	...
	EditorGUI.DrawRect(new Rect(16, rect.y + EditorGUIUtility.singleLineHeight + 5, rect.width + 22, EditorGUIUtility.singleLineHeight * 13f), new Color(0.9f, 0.9f, 0.9f, 1));	
	}
	else{
	GUI.color = new Color(0.4f, 0.45f, 0.4f, 1);
	EditorGUI.DrawRect(new Rect(16, rect.y + EditorGUIUtility.singleLineHeight + 5, rect.width + 22, EditorGUIUtility.singleLineHeight * 13f), new Color(0.8f, 0.85f, 0.9f, 1));	
	}
	...
	EditorGUI.LabelField(new Rect(rect.x, rect.y + (EditorGUIUtility.singleLineHeight * 6) + 30, (labelWidth * 2.5f) - 10, EditorGUIUtility.singleLineHeight), "Wallpaper");
	EditorGUI.LabelField(new Rect(rect.x, rect.y + (EditorGUIUtility.singleLineHeight * 10) + 40, (labelWidth * 2), EditorGUIUtility.singleLineHeight), "Requires resources");
	...
	EditorGUI.PropertyField(
		new Rect(rect.x + (labelWidth * 2.5f), rect.y + (EditorGUIUtility.singleLineHeight * 6) + 30, rect.width - (labelWidth * 2.5f), EditorGUIUtility.singleLineHeight),
        element.FindPropertyRelative("wallpaper"), GUIContent.none);
	
    element.FindPropertyRelative("description").stringValue = EditorGUI.TextArea(
		new Rect(rect.x, rect.y + (EditorGUIUtility.singleLineHeight * 7) + 35, rect.width, EditorGUIUtility.singleLineHeight * 3),
		element.FindPropertyRelative("description").stringValue);
		
	EditorGUI.PropertyField(
        new Rect(rect.x + (labelWidth * 2), rect.y + (EditorGUIUtility.singleLineHeight * 10) + 40, 15, EditorGUIUtility.singleLineHeight),
        element.FindPropertyRelative("requiresResources"), GUIContent.none);
		
	if(managerScript.pieces[index].requiresResources){
		EditorGUI.PropertyField(
			new Rect(rect.x + (labelWidth * 2) + 20, rect.y + (EditorGUIUtility.singleLineHeight * 10) + 40, labelWidth - 5, EditorGUIUtility.singleLineHeight),
			element.FindPropertyRelative("resourceAmount"), GUIContent.none);
			
		EditorGUI.PropertyField(
			new Rect(rect.x + (labelWidth * 3) + 20, rect.y + (EditorGUIUtility.singleLineHeight * 10) + 40, rect.width - ((labelWidth * 3) + 20), EditorGUIUtility.singleLineHeight),
	...
	list.elementHeightCallback = (index) => { 
		return EditorGUIUtility.singleLineHeight * 14.5f;
	};
