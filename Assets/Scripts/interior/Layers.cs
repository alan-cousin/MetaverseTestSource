...
public class Layers : MonoBehaviour {
	...
	void Start(){
		...
		defaultLayerText.GetComponent<Text>().text = PlayerPrefs.GetInt(index + " - " + "defaultLayer").ToString();
		
		manager = GameObject.FindObjectOfType<uBuildManager>();
	}
	...
	public void addLayers(){
		...
		for(int i = 0; i < PlayerPrefs.GetInt(index + " - " + "layerCount"); i++){
			...
			newLayer.GetComponentInChildren<InputField>().text = "" + PlayerPrefs.GetString(index + " - " + "L" + i + "N");
	...
	void updateLayerUI(){
		...
		for(int i = 0; i < layers.Count; i++){
			...
			if(manager.pieceSelected != null && manager.pieceSelected.GetComponent<PieceTrigger>().layer == 0){
			...
			//if the selected piece already has a layer...
			else if(manager.pieceSelected != null){
				...
				if(i == manager.pieceSelected.GetComponent<PieceTrigger>().layer - 1){
	...
	public void addLayer(){
		...
		PlayerPrefs.SetInt(index + " - " + "layer" + layers.Count, 0);
	}
	...
	public void removeLayer(){
		...
		if(EventSystem.current.currentSelectedGameObject.name == "Remove button"){
			...
			removeLayerWarning.SetActive(true);
				...
				if(layerToRemove + 1 == PlayerPrefs.GetInt(index + " - " + "defaultLayer")){
					PlayerPrefs.SetInt(index + " - " + "defaultLayer", 0);
					defaultLayerText.GetComponent<Text>().text = "0";
					StartCoroutine(defaultLayerResettedWarning());
				}
				else if(layerToRemove + 1 < PlayerPrefs.GetInt(index + " - " + "defaultLayer")){
					PlayerPrefs.SetInt(index + " - " + "defaultLayer", PlayerPrefs.GetInt(index + " - " + "defaultLayer") - 1);
					defaultLayerText.GetComponent<Text>().text = PlayerPrefs.GetInt(index + " - " + "defaultLayer").ToString();
				}
				//destroy layer ui
	...
	public void removeLayerDefault(){
		...
		foreach(GameObject piece in layers[layerToRemove].layerPieces){
			piece.GetComponent<PieceTrigger>().layer = PlayerPrefs.GetInt(index + " - " + "defaultLayer");
	...
	public void addToLayer(){
		...
		layers[layer].layerPieces.Add(manager.pieceSelected);
		//set piece layer (to save it correctly)
		manager.pieceSelected.GetComponent<PieceTrigger>().layer = layer + 1;
		}
	}
	...
	public void removeFromLayer(){
		...
		int layer = int.Parse(EventSystem.current.currentSelectedGameObject.transform.parent.gameObject.name);
		//remove piece from layer
		layers[layer].layerPieces.Remove(manager.pieceSelected);
		//reset piece layer
		manager.pieceSelected.GetComponent<PieceTrigger>().layer = 0;
		}
	}
	
	//move default layer up
	public void defaultLayerUp(){
		if(PlayerPrefs.GetInt(index + " - " + "defaultLayer") < layers.Count){
			PlayerPrefs.SetInt(index + " - " + "defaultLayer", PlayerPrefs.GetInt(index + " - " + "defaultLayer") + 1);
			defaultLayerText.GetComponent<Text>().text = PlayerPrefs.GetInt(index + " - " + "defaultLayer").ToString();
		}
	}
		...
		GameObject toggleObject = EventSystem.current.currentSelectedGameObject;
			...
			int toggledLayer = int.Parse(EventSystem.current.currentSelectedGameObject.transform.parent.gameObject.name);
				...
				if(toggleObject.GetComponent<Toggle>().isOn){
					...
					//save state
					PlayerPrefs.SetInt(index + " - " + "layer" + toggledLayer, 0);
				}
				else{
					...
					//save state
					PlayerPrefs.SetInt(index + " - " + "layer" + toggledLayer, 1);
				}
			}
		}
	}
